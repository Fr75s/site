...
{#if showOnJS}
<h1
    class="my-2 text-center font-display font-bold text-2xl text-planet-900 dark:text-planet-100"
>
    Filter
</h1>
...
<div class="max-w-7xl mx-auto flex justify-center gap-2">
    {#each tagList as tag}
        <button
            ...
            class="block align-middle py-2 px-5 rounded-full font-bold text-sm text-planet-900 
            drop-shadow-md border transition-colors {tagFilter == tag ? 'border-planet-900 dark:border-planet-100' : 'border-planet-900/0 dark:border-planet-100/0'}"
